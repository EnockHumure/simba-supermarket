...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = (email: string, password: string) => {
    ...
    const updatedProfile = {
      ...
      clippedCouponIds: profile.clippedCouponIds || [],
    };
  ...
  const toggleCoupon = (couponId: string) => {
    ...
    const currentCoupons = user.clippedCouponIds || [];
    const isClipped = currentCoupons.includes(couponId);
    
    const updatedCoupons = isClipped 
      ? currentCoupons.filter(id => id !== couponId)
      : [...currentCoupons, couponId];
      
    updateProfile(user.email, { clippedCouponIds: updatedCoupons });
  };
