...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const { mode, location, resetFulfillment } = useFulfillment();
  ...
  return (
    <header className="sticky top-0 z-[50] bg-white border-b border-simba-line shadow-sm">
      <nav className="max-w-[1200px] mx-auto px-4 h-20 flex items-center gap-6" aria-label="Main Navigation">
        ...
        <div className="hidden lg:flex flex-col gap-0.5" onClick={resetFulfillment} role="button" aria-label="Change Fulfillment Method">
          <span className="text-[10px] font-black uppercase tracking-widest text-simba-muted">
            {mode === 'delivery' ? '🚚 Delivering to' : '🏪 Picking up at'}
          </span>
          <div className="flex items-center gap-1 group cursor-pointer">
            <span className="text-sm font-black text-simba-ink group-hover:text-simba-primary transition-colors">
              {location || 'Select method'}
            </span>
            <span className="text-[10px] text-simba-muted group-hover:text-simba-primary">▼</span>
          </div>
        </div>
