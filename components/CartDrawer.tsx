...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose, onCheckout, selectedLocation }) => {
  ...
  const [checkoutStep, setCheckoutStep] = useState<'cart' | 'fulfillment' | 'details'>('cart');
  ...
  const handleCheckoutClick = () => {
    if (checkoutStep === 'cart') {
      setCheckoutStep('fulfillment'); // Show fulfillment mode selection first
  ...
  const handleFulfillmentModeSelect = (mode: 'delivery' | 'pickup' | 'in-store') => {
    // Store selected mode and proceed to payment details
    console.log('Selected fulfillment mode:', mode);
    setCheckoutStep('details');
  };
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {cart.length === 0 ? (
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-black text-simba-ink uppercase tracking-wider">How would you like to receive your order?</h3>
                
                {[
                  { id: 'delivery', icon: '🚗', title: t('delivery'), desc: t('deliveryDescription'), fee: '2,000 RWF' },
                  { id: 'pickup', icon: '🏪', title: t('pickup'), desc: t('pickupDescription'), fee: 'Free' },
                  { id: 'in-store', icon: '🏪', title: t('inStore'), desc: t('inStoreDescription'), fee: 'Free' },
                ].map((mode) => (
                  <button
                    key={mode.id}
                    className="w-full flex items-center gap-4 p-4 rounded-xl border-2 transition-all text-left hover:border-simba-primary hover:bg-simba-primary/5
                      {checkoutStep === 'details' && mode.id === 'delivery' ? 'border-simba-primary bg-simba-primary/10' : 'border-simba-line bg-white'}
                    "
                    onClick={() => handleFulfillmentModeSelect(mode.id as 'delivery' | 'pickup' | 'in-store')}
                  >
                    <span className="w-12 h-12 bg-simba-bg rounded-lg flex items-center justify-center text-2xl">{mode.icon}</span>
                    <div className="flex-1">
                      <strong className="text-sm font-black text-simba-ink block">{mode.title}</strong>
                      <p className="text-[10px] text-simba-muted font-medium mt-0.5">{mode.desc}</p>
                    </div>
                    <span className="text-xs font-bold text-simba-primary">{mode.fee}</span>
                  </button>
                ))}
              </div>
            </div>
