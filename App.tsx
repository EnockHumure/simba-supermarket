...
const AppContent: React.FC = () => {
  ...
  const { mode: fulfillmentMode, location: fulfillmentLocation, resetFulfillment } = useFulfillment();
  ...
  const [selectedFulfillmentMode, setSelectedFulfillmentMode] = useState<FulfillmentMode>('delivery');
  const [deliveryAddress, setDeliveryAddress] = useState('');
  const [deliveryLocation, setDeliveryLocation] = useState('Kigali CBD');
  ...
  const products = useMemo(() => {
    return allProducts.filter((p: Product) => {
      ...
      const matchesDepartment = selectedDepartment ? p.departmentId === selectedDepartment : true;
      return matchesSearch && matchesDepartment;
    });
  }, [allProducts, searchTerm, selectedDepartment]);
  ...
  const handleCheckout = (branchName: string, pickupTime: string, paymentMethod: string, momoDeposit?: number) => {
    ...
    // Show fulfillment modal first
    setSelectedFulfillmentMode('delivery');
    setDeliveryAddress('');
    setDeliveryLocation('Kigali CBD');
    setDeliveryTime('');
  ...
  const handleFulfillmentModeSelect = (mode: FulfillmentMode) => {
    ...
    // Note: paymentMethod and momoDeposit would normally come from the CartDrawer/Checkout flow
    ...
    checkout(
      ...
      0,
      mode,
      mode === 'delivery' ? deliveryAddress : undefined,
      mode === 'delivery' ? deliveryLocation : undefined,
      mode === 'delivery' ? deliveryTime : undefined
    );
    ...
    <div className="min-h-screen bg-simba-bg">
      ...
      <main className="max-w-[1200px] mx-auto px-4 py-8 flex flex-col gap-12">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          <Sidebar 
            selectedCategoryId={selectedDepartment}
            onSelectCategory={(deptId) => {
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-8" ref={productSectionRef}>
            <MyItemsSection onProductClick={setSelectedProduct} viewMode={viewMode} />
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-simba-line pb-6">
              <div>
                ...
                <h2 className="text-3xl font-black text-simba-ink mt-1">
                  {selectedDepartment ? translateCategoryLabel(selectedDepartment, language) : t('everythingOnSimba')}
                </h2>
              </div>
              ...
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                <div className="ml-auto flex items-center gap-3">
                  ...
                  {selectedDepartment && (
                    <button className="text-xs font-black text-simba-primary underline" onClick={() => setSelectedDepartment(null)}>
                      {t('resetFilter')}
                    </button>
                  )}
                </div>
